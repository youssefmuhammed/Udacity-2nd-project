...
const showProduct = async (req: Request, res: Response) => {
    ...
    try {
        const product = await database.showProduct(req.params.id as unknown as number)
        res.json(product);
    } catch (err) {
       res.status(400)
       res.json(err)
...
const deleteProduct = async (req: Request, res: Response) => {
    ...
    try {
        const deletedProduct = await database.deleteProduct(req.params.id as unknown as number)
